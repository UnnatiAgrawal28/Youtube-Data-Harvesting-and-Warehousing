...
#Connect SQL
connection_obj= sqlite3.connect("Youtube_dataharvesting.db")
...
def data_from_videoinfo(channel_stats):
    ...
    dtype_dict = {'channel_id': 'VARCHAR', 'Video_id': 'VARCHAR', 'Video_name': 'TEXT', "Description": 'TEXT',
                  ...
                  'Fav_count': 'INTEGER', 'Comment_count': 'INTEGER', 'Duration': "TIME", 'Caption_status': 'TEXT'}
    ...
    df_to_upload = df_cleaned[~df_cleaned["Video_id"].astype(str).isin(existing_video_ids)]
...
if result:
    ...
    st.warning(f"No data found for the selected channel name: {ch_name}")
#--------------------------------------------------------------------------x----------------------------------------------------------------------#
###Main Questions
